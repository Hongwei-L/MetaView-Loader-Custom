...
//
// Portions based on unity-xr-plugin/Providers/Display/Display.h
...
class OpenVRDisplayProvider {
  public:
    ...
    UnitySubsystemErrorCode Lifecycle_Initialize(UnitySubsystemHandle handle,
                                                 void *userData);
    ...
    UnitySubsystemErrorCode GfxThread_BlitToMirrorViewRenderTarget(
        const UnityXRMirrorViewBlitInfo *mirrorBlitInfo);
    UnitySubsystemErrorCode GfxThread_Stop();

    UnitySubsystemErrorCode MainThread_UpdateDisplayState(
        UnityXRDisplayState *state);
    UnitySubsystemErrorCode MainThread_QueryMirrorViewBlitDesc(
    ...
    /// Get the currently active mirror mode - kUnityXRMirrorBlitNone,
    /// kUnityXRMirrorBlitLeftEye, kUnityXRMirrorBlitRightEye,
    /// kUnityXRMirrorBlitDistort/SteamVR View (default)
    int GetCurrentMirrorMode() const { return m_nMirrorMode; }

    /// Set active mirror mode - kUnityXRMirrorBlitNone,
    /// kUnityXRMirrorBlitLeftEye, kUnityXRMirrorBlitRightEye,
    /// kUnityXRMirrorBlitDistort/SteamVR View (default)
  ...
  private:
    ...
    /// @param[in] eEye - For which eye to process the texture
    /// @param[in] stage - The stage for this frame to pull the correct
    /// texture from m_NativeColorTextures
    /// @return bool - If the submit to the compositor succeeded
    bool SubmitToCompositor(EEye eEye, int nStage);

    /// Set the render pass properties and parameters that will be used in the
    /// target frame
    /// @param[in] eEye - Which eye to set this render param for
    /// @param[in] frameHints - Frame info
    /// @param[in] nextFrame - The target frame
    void SetupRenderPass(const EEye eEye,
                         const UnityXRFrameSetupHints *pFrameHints,
                         UnityXRNextFrameDesc *pTargetFrame);

    /// Set the occlusion mesh (hidden area mesh) for a given eye
    /// @param[in] eEye - Target eye for the occlusion mesh
    /// @return bool - If we got an occlusion mesh properly setup for the target
    /// eye
    UnityXROcclusionMeshId SetupOcclusionMesh(EEye eEye);

    /// Get the distance squared between two Vectors
    /// @param[in] pVector1 - The origin vector
    /// @param[in] pVector2 - The target vector
    ...
    /// @return UnityXRVector2 - The recommended width (x) and height
    /// (y) of the mirror view
    const UnityXRVector2 GetRecommendedMirrorResolution();

    /// Get the Eye to Head transform for the given eye
    /// @param[in] eye - 0:Left, 1:Right
    /// @return UnityXRPose - Position and Rotation (quaternion)
    UnityXRPose GetEyePose(EEye eye);

    /// Helper function to calculate the projection matrix for a given eye
    /// @param[in] eye - 0:Left, 1:Right
    /// @param[in] flNear - the near projection value (clipping area) of
    /// the camera (can be changed during runtime)
    /// @param[in] flNear - the far projection value (clipping area) of
    /// the camera (can be changed during runtime)
    /// @return UnityXRProjection
    UnityXRProjection GetProjection(EEye eye, float flNear, float flFar);
    ...
    /// @param[in] const UnityXRFrameSetupHints* frameHint - Details about the
    /// frame (frame number, frame in flight, singlepass, etc)
    /// @return UnitySubsystemErrorCode
    UnitySubsystemErrorCode CreateEyeTextures(
        const UnityXRFrameSetupHints *frameHints);

    /// Destroy the textures Unity uses to submit to the compositor
    /// @param[in] UnitySubsystemHandle handle - The handle for this display
    /// provider
    ...
    /// @return The device native texture that needs to be sent to the
    /// compositor
    ...
    /// Submit to the metaview::Renderer.
    void SubmitToRenderer(int stage);

    /// Get eye texture dimensions, estimated if the render is not yet up.
    void GetEyeTextureDimensions(uint32_t &height, uint32_t &width) const;

    /// The occlusion mesh (hidden area mesh) handle for the left eye. 0 if
    /// none.
    UnityXROcclusionMeshId m_pOcclusionMeshLeftEye = 0;

    /// The occlusion mesh (hidden area mesh) handle for the right eye. 0 if
    /// none.
    ...
    /// Cache for the mirror textures. This will be populated by the "VR View"
    /// textures from SteamVR
    ...
    /// Defines the part of the provided texture that will be used in the frame
    /// buffer
    ...
    uint32_t m_nEyeWidth, m_nEyeHeight, m_nEyeMirrorWidth, m_nEyeMirrorHeight,
        m_nRenderMirrorWidth, m_nRenderMirrorHeight;

    /// The sub-rect of the mirror texture that we need to pass down to Unity's
    /// mirror blit
    ...
    /// If sRGB is used
    bool m_bIsUsingSRGB = false;

    /// Whether the current frame is in flight (ready, queued for gpu)
    bool m_bFrameInFlight = false;

    /// Flag to indicate whether all Unity side textures have been created
    /// (triggers CreateEyeTextures if not)
    bool m_bTexturesCreated = false;

    /// Whether the application is using single pass or multi pass (can be
    /// changed in runtime)
    bool m_bUseSinglePass = false;

    /// Whether eyes should be rotated before rendering for scanout purposes.
    bool m_bRotateEyes = true;

    EVRStereoRenderingModes m_renderingMode{EVRStereoRenderingModes::MultiPass};
    ...
    /// Holds the OpenVR Vulkan Texture Array data for submitting to the
    /// compositor
    // vr::VRVulkanTextureArrayData_t m_vrVulkanTexture = {};

    /// Holds the Unity equivalent eye textures per stage (0:Left, 1: Right,
    /// Single Pass only uses left with texture array size of 2)
    UnityXRRenderTextureId m_UnityTextures[k_nMaxNumStages][2];

    std::unique_ptr<metaview::Renderer> renderer_;
    std::vector<winrt::com_ptr<ID3D11Texture2D>> swapchainImages_;
    std::vector<winrt::com_ptr<ID3D11RenderTargetView>> rtvs_;
    int swapchainImageIndex_ = -1;
};
